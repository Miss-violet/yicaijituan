...
class List extends Component {
  ...
  render() {
    const { listData, rowSelection, handleTableChange, showModal, handleChangeStatus, selectedStatus, selectedId, role } = this.props;
    const btnBar = (
          ...
          (role === '0' || role === '1') && (
            <span>
              ...
              <Button className={styles.btn} onClick={handleChangeStatus}>
                {
                  selectedStatus === 0 ? (<span><Icon type="minus" />停用</span>) : (<span><Icon type="check" />启用</span>)
                }
              </Button>
              ...
              </Button>
              {
                selectedId === undefined ? (
                  <Tooltip title="请先选中一条单据后再查看">
                    <span className={styles.btn}><Icon type="printer" /> 查看合格证</span>
                  </Tooltip>
                ) : selectedStatus === 0 && (
                  <Link target="_blank" to={`/report/${selectedId}`} className={styles.btn}><Icon type="printer" /> 查看合格证</Link>
                )
              }
            </span>
          )}
    ...
    const columns = [
      {
        ...
        fixed: 'left',
        render: (text, record) => {
          if (record.status === 1) return <span><Tooltip title="此出库单已被停用"><Badge status="error" /></Tooltip>{text}</span>
